# I AM 18 counter server.py

# THIS NERO SENECA AGRIPPINA IS TO CREATE NARRATIVE ASSOCIATIONS WITH 
#   WITH PICTURES AND VISUAL AIDS
#       SO I CAN NARRATIVE AND RECOGNIZE PATTERNS
#          THIS IS A MNEMONIC DEVICE

# ALSO I didn't undrstand the session box visual until the next assignment
...
# Not using 'request' from below, what would it be doing?
            # session is a type of tracking, like cookies
            # if something goes in session, it stays until 
...
def index():
    ...
    if "count" in session:
        ...
        session["count"] = 1   #this line creates the key-value pair, creates key count, assigns value 1
    print("==================================end index")

                            # is passing information to the all html
                            #  line 20 on 04_index.html is where
                            #  the info gets passed in 
...
def form():
    print("==================================beginning ")
    # I need a diagram of how this is counting, and what session is
                        # key from form, value is the input
    session["count"] = request.form["number_input"]
...
def seneca_the_stoic():
    ...
    if "count" in session:
        ...
        session["count"] = 1
    return render_template("z06_seneca.html", count = session["count"])
...
def seneca_the_mathematician():
    ...
    return render_template("z07_seneca.html", count = session["count"])

@app.route('/seneca-adds-5')
def seneca_the_mathematician_5():
    # This adds to the count on index page, but does not display in either
    # seneca page
    print("==================================beginning Seneca adds 5")
    if "count" in session:
        session["count"] +=5 
    else:
        session["count"] = 1

    print("==================================end Seneca adds 5")
    return render_template("z07_seneca.html", count = session["count"])

@app.route("/seneca-adds-5-back")
def seneca_the_mathematician_5_back():
    # This adds to the count on index page, but does not display in either
    # seneca page
    print("==================================beginning Seneca adds 5 goes back")
    if "count" in session:
        session["count"] +=4 
        # 
    else:
        session["count"] = 1
    print("==================================end Seneca adds 5 goes back")
    return redirect("/")
